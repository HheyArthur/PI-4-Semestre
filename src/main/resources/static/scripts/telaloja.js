                ...
                if (produto.ativo) { // Verifica se o produto está ativo
                    var produtoCard = `
                        <div class="produto">
                            ...
                            <button class="btn-comprar" data-produto-id="${produto.id}" onclick="adicionarAoCarrinho(${produto.id}, '${produto.nomeProduto}', ${produto.preco}, '${produto.imagemPrincipal}', '${produto.descricao}')">Adicionar ao Carrinho</button>
                        </div>
                    `;
...
function adicionarAoCarrinho(id, nome, preco, imagem, descricao) {
    ...
    if (produto) {
        ...
        carrinho.push({ id: id, nome: nome, preco: preco, quantidade: 1, imagem: imagem, descricao: descricao});
    }
